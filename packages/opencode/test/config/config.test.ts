  ...
  const authWellKnown = Layer.mock(AuthWellKnown.Service)({
    configs: () =>
      Effect.gen(function* () {
        ...
        const env: Record<string, string> = { TEST_TOKEN: "test-token" }
        ...
        if (
          typeof metadata === "object" &&
          metadata !== null &&
          "remote_config" in metadata &&
          typeof metadata.remote_config === "object" &&
          metadata.remote_config !== null &&
          "url" in metadata.remote_config &&
          typeof metadata.remote_config.url === "string"
        ) {
          const remoteURL = metadata.remote_config.url.replace(
            /\{env:([^}]+)\}/g,
            (_, key) => env[key] ?? process.env[key] ?? "",
          )
          const headers =
            "headers" in metadata.remote_config &&
            typeof metadata.remote_config.headers === "object" &&
            metadata.remote_config.headers !== null
              ? Object.fromEntries(
                  Object.entries(metadata.remote_config.headers)
                    .filter((entry): entry is [string, string] => typeof entry[1] === "string")
                    .map(([key, value]) => [
                      key,
                      value.replace(/\{env:([^}]+)\}/g, (_, envKey) => env[envKey] ?? process.env[envKey] ?? ""),
                    ]),
                )
              : undefined
          ...
          const remote = input.remote as Record<string, unknown>
          const content = typeof remote.config === "object" && remote.config !== null ? remote.config : remote
          docs.push({
            url: remoteURL,
            source: remoteURL,
